...
async function fixProductionDatabase() {
  ...
  try {
    ...
    // Check and add comment column to withdrawals
    const commentCheck = await pool.query(`
      SELECT column_name 
      FROM information_schema.columns 
      WHERE table_name = 'withdrawals' AND column_name = 'comment'
    `);
    
    if (commentCheck.rows.length === 0) {
      await pool.query(`ALTER TABLE withdrawals ADD COLUMN comment TEXT`);
      console.log('✅ Added comment column to withdrawals table');
    } else {
      console.log('✓ Comment column already exists in withdrawals');
    }
