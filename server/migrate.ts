  ...
  try {
    ...
    await db.execute(sql`
      CREATE TABLE IF NOT EXISTS videos (
        ...
        thumbnail_url TEXT,
        price_in_sats INTEGER DEFAULT 0 NOT NULL,
    ...
    `);
    console.log('✅ [MIGRATION] videos table ensured');
    ...
    `);
    console.log('✅ [MIGRATION] video_unlocks table ensured');
    ...
    `);
    console.log('✅ [MIGRATION] video_reactions table ensured');
    ...
    `);
    console.log('✅ [MIGRATION] video_comments table ensured');
    ...
    `);
    console.log('✅ [MIGRATION] video_saves table ensured');

    // Indexes for video tables
    await db.execute(sql`CREATE INDEX IF NOT EXISTS idx_videos_is_active ON videos(is_active)`);
    await db.execute(sql`CREATE INDEX IF NOT EXISTS idx_videos_created_at ON videos(created_at DESC)`);
    await db.execute(sql`CREATE INDEX IF NOT EXISTS idx_video_unlocks_user_id ON video_unlocks(user_id)`);
    await db.execute(sql`CREATE INDEX IF NOT EXISTS idx_video_comments_video_id ON video_comments(video_id)`);
