...
🤖 Bot: @${_admBotName}`;
  ...
  // GET /api/videos - list all active videos for the feed
  app.get('/api/videos', authenticateTelegram, async (req: any, res) => {
    try {
      const user = req.user?.user;
      const userId = user?.id;

      const allVideos = await db.select().from(videos).where(eq(videos.isActive, true)).orderBy(desc(videos.createdAt));

      let unlockedVideoIds: Set<string> = new Set();
      if (userId) {
        const unlocks = await db.select({ videoId: videoUnlocks.videoId }).from(videoUnlocks).where(eq(videoUnlocks.userId, userId));
        unlocks.forEach(u => unlockedVideoIds.add(u.videoId));
      }

      let userReactions: Map<string, string> = new Map();
      let savedVideoIds: Set<string> = new Set();
      if (userId) {
        const reactions = await db.select({ videoId: videoReactions.videoId, type: videoReactions.reactionType }).from(videoReactions).where(eq(videoReactions.userId, userId));
        reactions.forEach(r => userReactions.set(r.videoId, r.type));

        const saves = await db.select({ videoId: videoSaves.videoId }).from(videoSaves).where(eq(videoSaves.userId, userId));
        saves.forEach(s => savedVideoIds.add(s.videoId));
      }

      const result = allVideos.map(v => ({
        ...v,
        isUnlocked: v.priceInSats === 0 || unlockedVideoIds.has(v.id),
        userReaction: userReactions.get(v.id) || null,
  ...
  app.get('/api/videos/:id', authenticateTelegram, async (req: any, res) => {
    try {
      const { id } = req.params;
      const user = req.user?.user;
      const userId = user?.id;
      ...
      let isUnlocked = video.priceInSats === 0;
      let userReaction = null;
      let isSaved = false;

      if (userId) {
        const [unlock] = await db.select().from(videoUnlocks).where(and(eq(videoUnlocks.userId, userId), eq(videoUnlocks.videoId, id)));
        if (unlock) isUnlocked = true;

        const [reaction] = await db.select().from(videoReactions).where(and(eq(videoReactions.userId, userId), eq(videoReactions.videoId, id)));
        if (reaction) userReaction = reaction.reactionType;

        const [save] = await db.select().from(videoSaves).where(and(eq(videoSaves.userId, userId), eq(videoSaves.videoId, id)));
        if (save) isSaved = true;
      }

      return res.json({ success: true, video: { ...video, isUnlocked, userReaction, isSaved } });
    } catch (error) {
  ...
  // POST /api/videos/:id/unlock - unlock a video by spending sats
  app.post('/api/videos/:id/unlock', authenticateTelegram, async (req: any, res) => {
    try {
      const { id } = req.params;
      const user = req.user?.user;
      ...
      const [existing] = await db.select().from(videoUnlocks).where(and(eq(videoUnlocks.userId, user.id), eq(videoUnlocks.videoId, id)));
      if (existing || video.priceInSats === 0) {
        return res.json({ success: true, message: 'Already unlocked', newBalance: user.balance });
      }

      const userBalance = parseFloat(user.balance || '0');
      if (userBalance < video.priceInSats) {
        return res.status(400).json({ success: false, message: `Insufficient balance. You need ${video.priceInSats} SAT.` });
      }

      // Deduct sats and unlock
      await db.update(users).set({ balance: sql`${users.balance} - ${video.priceInSats}` }).where(eq(users.id, user.id));
      await db.insert(videoUnlocks).values({ userId: user.id, videoId: id });

      // Increment views
      await db.update(videos).set({ views: sql`${videos.views} + 1`, updatedAt: new Date() }).where(eq(videos.id, id));

      const [updatedUser] = await db.select().from(users).where(eq(users.id, user.id));
      return res.json({ success: true, message: 'Video unlocked!', newBalance: updatedUser?.balance });
    } catch (error) {
      console.error('Error unlocking video:', error);
      return res.status(500).json({ success: false, message: 'Failed to unlock video' });
    }
  });

  // POST /api/videos/:id/view - increment view count (for free/already-unlocked videos)
  app.post('/api/videos/:id/view', authenticateTelegram, async (req: any, res) => {
    try {
      const { id } = req.params;
      await db.update(videos).set({ views: sql`${videos.views} + 1`, updatedAt: new Date() }).where(eq(videos.id, id));
  ...
  // POST /api/videos/:id/react - like or dislike
  app.post('/api/videos/:id/react', authenticateTelegram, async (req: any, res) => {
    try {
      const { id } = req.params;
      ...
      const [existing] = await db.select().from(videoReactions).where(and(eq(videoReactions.userId, user.id), eq(videoReactions.videoId, id)));
      ...
      if (existing) {
        if (existing.reactionType === type) {
          ...
          if (type === 'like') {
            await db.update(videos).set({ likesCount: sql`GREATEST(0, ${videos.likesCount} - 1)` }).where(eq(videos.id, id));
          } else {
            await db.update(videos).set({ dislikesCount: sql`GREATEST(0, ${videos.dislikesCount} - 1)` }).where(eq(videos.id, id));
          }
          return res.json({ success: true, reaction: null });
        } else {
          // Change reaction
          await db.update(videoReactions).set({ reactionType: type }).where(eq(videoReactions.id, existing.id));
          if (type === 'like') {
            await db.update(videos).set({ likesCount: sql`${videos.likesCount} + 1`, dislikesCount: sql`GREATEST(0, ${videos.dislikesCount} - 1)` }).where(eq(videos.id, id));
          } else {
            await db.update(videos).set({ dislikesCount: sql`${videos.dislikesCount} + 1`, likesCount: sql`GREATEST(0, ${videos.likesCount} - 1)` }).where(eq(videos.id, id));
          }
          return res.json({ success: true, reaction: type });
        }
      } else {
        await db.insert(videoReactions).values({ userId: user.id, videoId: id, reactionType: type });
        if (type === 'like') {
          await db.update(videos).set({ likesCount: sql`${videos.likesCount} + 1` }).where(eq(videos.id, id));
        } else {
          await db.update(videos).set({ dislikesCount: sql`${videos.dislikesCount} + 1` }).where(eq(videos.id, id));
        }
        return res.json({ success: true, reaction: type });
      }
    } catch (error) {
  ...
  app.post('/api/videos/:id/comment', authenticateTelegram, async (req: any, res) => {
    try {
      const { id } = req.params;
      ...
      const [comment] = await db.insert(videoComments).values({ userId: user.id, videoId: id, content: content.trim() }).returning();
      await db.update(videos).set({ commentsCount: sql`${videos.commentsCount} + 1` }).where(eq(videos.id, id));

      return res.json({ success: true, comment: { ...comment, username: user.username || user.firstName || 'User' } });
    } catch (error) {
  ...
  app.get('/api/videos/:id/comments', authenticateTelegram, async (req: any, res) => {
    try {
      const { id } = req.params;
  ...
  // POST /api/videos/:id/save - save or unsave a video
  app.post('/api/videos/:id/save', authenticateTelegram, async (req: any, res) => {
    try {
      const { id } = req.params;
      const user = req.user?.user;
      if (!user) return res.status(401).json({ success: false, message: 'Not authenticated' });

      const [existing] = await db.select().from(videoSaves).where(and(eq(videoSaves.userId, user.id), eq(videoSaves.videoId, id)));

      if (existing) {
        await db.delete(videoSaves).where(eq(videoSaves.id, existing.id));
        return res.json({ success: true, saved: false });
      } else {
        await db.insert(videoSaves).values({ userId: user.id, videoId: id });
  ...
  // GET /api/user/saved-videos - get user's saved videos
  app.get('/api/user/saved-videos', authenticateTelegram, async (req: any, res) => {
    try {
      const user = req.user?.user;
      if (!user) return res.status(401).json({ success: false, message: 'Not authenticated' });
      ...
      const saved = await db
        .select({ video: videos })
        .from(videoSaves)
        .innerJoin(videos, eq(videoSaves.videoId, videos.id))
        .where(and(eq(videoSaves.userId, user.id), eq(videos.isActive, true)))
        .orderBy(desc(videoSaves.savedAt));

      return res.json({ success: true, videos: saved.map(s => ({ ...s.video, isSaved: true })) });
    } catch (error) {
      console.error('Error fetching saved videos:', error);
      return res.status(500).json({ success: false, message: 'Failed to fetch saved videos' });
    }
  });

  // ADMIN: GET /api/admin/videos - list all videos
  app.get('/api/admin/videos', authenticateAdmin, async (req: any, res) => {
    try {
      const allVideos = await db.select().from(videos).orderBy(desc(videos.createdAt));
      return res.json({ success: true, videos: allVideos });
    } catch (error) {
  ...
  // ADMIN: POST /api/admin/videos - create a new video
  app.post('/api/admin/videos', authenticateAdmin, async (req: any, res) => {
    try {
      const telegramUser = req.user?.telegramUser;
      const { title, description, videoUrl, thumbnailUrl, priceInSats } = req.body;
      if (!title?.trim() || !videoUrl?.trim()) {
        return res.status(400).json({ success: false, message: 'Title and video URL are required' });
      }

      // Find admin user in DB by telegram_id
      let adminId = 'admin';
      if (telegramUser?.id) {
        const [adminUser] = await db.select({ id: users.id }).from(users).where(eq(users.telegramId, telegramUser.id.toString()));
      ...
      const [video] = await db.insert(videos).values({
        ...
        priceInSats: parseInt(priceInSats) || 0,
        isActive: true,
      }).returning();
  ...
  // ADMIN: PUT /api/admin/videos/:id - update a video
  app.put('/api/admin/videos/:id', authenticateAdmin, async (req: any, res) => {
    try {
      const { id } = req.params;
      const { title, description, videoUrl, thumbnailUrl, priceInSats, isActive } = req.body;
  ...
  // ADMIN: DELETE /api/admin/videos/:id - delete a video
  app.delete('/api/admin/videos/:id', authenticateAdmin, async (req: any, res) => {
    try {
      const { id } = req.params;
